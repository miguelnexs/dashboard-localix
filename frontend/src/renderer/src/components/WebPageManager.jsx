...
const WebPageManager = ({ token, apiBase, setView, setProductEditing }) => {
        ...
        <div className="space-y-3">
          <div className="bg-white/5 border border-white/10 rounded p-4">
            <div className="flex items-center justify-between">
              ...
              {settings.site_url && (<a href={settings.site_url} target="_blank" rel="noreferrer" className="text-blue-400 hover:text-blue-300 text-sm">Abrir sitio</a>)}
            </div>
            <div className="text-xs text-gray-300">Acceso sin inicio de sesión.</div>
          </div>
          <PortalContent token={token} apiBase={apiBase} />
        </div>
      )}
...
export const PortalContent = ({ token, apiBase }) => {
  ...
  const load = async () => {
    ...
    try {
      const res = await fetch(`${apiBase}/webconfig/public/portal/`, { headers: headers(null) });
