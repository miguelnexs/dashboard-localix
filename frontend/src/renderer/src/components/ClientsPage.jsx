                  ...
                  {items.map((c) => (
                    <tr key={c.id} className="border-t border-gray-700">
                      ...
                      <td className="px-3 py-2">
                        <div className="flex items-center gap-2">
                          <button onClick={async () => { try { const res = await fetch(`${apiBase}/clients/orders/${c.id}/`, { headers: authHeaders(token) }); const data = await res.json(); if (res.ok) { setViewClient(data.client); setViewOrders(data.orders || []); } } catch(_){} }} className="px-2 py-1 text-xs rounded bg-indigo-600 hover:bg-indigo-700 text-white">Ver</button>
