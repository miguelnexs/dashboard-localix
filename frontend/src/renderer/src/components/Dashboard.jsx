...
const Dashboard = ({ token, role, onSignOut }) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-gray-950 flex">
        <Sidebar view={view} setView={(v) => { setView(v); if (v === 'pedidos') { const headers = { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }; fetch(`${apiBase}/sales/notifications/read/`, { method: 'POST', headers }).then(() => setOrderNotif(0)).catch(() => setOrderNotif(0)); } }} onSignOut={onSignOut} role={role} orderNotif={orderNotif} />
      <main className="flex-1 p-6 space-y-6">
        ...
        {view === 'web' && (
          <WebPageManager token={token} apiBase={apiBase} setView={(v) => setView(v)} setProductEditing={(p) => { setProductEditing(p); setView('producto_form'); }} />
        )}
        {view === 'pedidos' && (
